...
import type { DshSessionId, ResolvedBackgroundTasksConfig } from './types.js'
 ...
 */
const FALLBACK_SESSION_ID = 'default' as unknown as DshSessionId
  ...
  disposers.push(
    ctx.tools.register(
      defineTool({
        ...
        description:
          'Run a shell command (PowerShell on Windows via -EncodedCommand, Bash on Linux/macOS). ' +
          `If the command exits within wait_ms (default ${config.waitMsBeforeAsync}ms), the exit code and output are returned synchronously. ` +
          'If it takes longer, it is automatically promoted to a background task and a taskId is returned. ' +
          'When a promoted background task finishes, a system notification with its output tail is delivered automatically.',
        parameters: {
          command: {
            type: 'string',
            required: true,
        ...
        async execute(args, exec) {
          ...
          if (result.mode === 'sync') {
            ...
            const display =
              body.length <= SYNC_DISPLAY_LIMIT_BYTES
                ? body
        ...
        async execute(args) {
          if (args.action === 'list') {
            ...
            const lines = tasks.map(
              (t) =>
                `- [${t.taskId}] (${t.status}) cmd: "${t.command}" | log: ${t.logPath}`,
            )
          ...
          if (args.action === 'logs') {
            ...
            if (!logResult.exists) return `Error: Task "${args.task_id}" not found.`
          ...
          return `Error: Unknown action "${args.action satisfies never}".`
        },
      }),
    ),
  )
