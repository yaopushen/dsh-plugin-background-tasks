 ...
 * mounted). Standing clauses are rationed: each must map to a reproduced
 * real-world failure with no deterministic engineering fix. The SSH quoting
 * rule lives in the tool description (tool-scoped); the wait-window rule died
 * with the wait_ms parameter.
 */
