...
模型侧观察：长命令晋升后，模型习惯立刻以 `job_output(wait: true)` 连续轮询，直到等待上限逐次到期返回。0.0.2 在插件自有的两个模型可见面注入明确指引（官方 `[status: ...]` 收尾行与 jobs 消费面零改动）：

- 晋升消息追加一行指引：不要轮询；继续独立工作或结束本轮，完成通知会自动唤醒。
- 晋升任务的每次未结束读取在正文末尾携带固定提示行（`renderProcessRead` 新增 `stillRunning` 参数）；同步完成路径与已结算读取保持无提示。
- 质量：纯适配层 30 用例 + 编排层集成 18 用例全绿。
