 ...
 */
export function deliverTaskWakeup(ctx, task, tailLogs) {
    const log = ctx.logger('background-tasks');
    const agent = ctx.agents.get(task.agentId);
    if (!agent) {
        log.warn('background-tasks: agent %s is no longer active; dropping wakeup for task %s (full log kept at %s)', task.agentId, task.taskId, task.logPath);
        return;
    }
    ...
    try {
        ...
        log.info('background-tasks: delivered wakeup for task %s to session %s', task.taskId, task.sessionId);
    }
    catch (err) {
        ...
        log.warn('background-tasks: failed to deliver wakeup for task %s: %s', task.taskId, String(err));
    }
}
