...
export const BACKGROUND_TASKS_DEFAULTS = {
...
function requireInt(raw, field, fallback, { min }) {
    if (raw === undefined)
        return fallback;
    if (typeof raw !== 'number' || !Number.isInteger(raw) || raw < min) {
        throw new TypeError(`background-tasks: config.${field} must be an integer >= ${min}, got ${JSON.stringify(raw)}`);
    }
    return raw;
}
function requireString(raw, field, fallback) {
    if (raw === undefined)
        return fallback;
    if (typeof raw !== 'string' || raw.trim() === '') {
        throw new TypeError(`background-tasks: config.${field} must be a non-empty string, got ${JSON.stringify(raw)}`);
    }
    return raw;
}
/**
 ...
 */
export function resolveBackgroundTasksConfig(raw = {}) {
    if (raw === null || typeof raw !== 'object' || Array.isArray(raw)) {
        throw new TypeError(`background-tasks: config must be an object, got ${JSON.stringify(raw)}`);
    }
    return {
