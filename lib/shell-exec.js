 ...
 */
export function resolveWorkdir(modelCwd, headerCwd, policyWorkspaceRoot) {
    const sessionCwd = policyWorkspaceRoot ?? (headerCwd === undefined ? undefined : canonicalPath(headerCwd));
    if (modelCwd === undefined)
        return sessionCwd;
 ...
 */
export function renderProcessRead(read, sandbox, escalationAvailable = false, stillRunning = false) {
    let text = read.delta;
    if (read.lossy) {
 ...
 */
export async function raceCompletion(proc, waitMs, abort) {
    let timer;
    let onAbort;
    const abortArm = new Promise((resolve) => {
        if (abort === undefined)
            return;
        if (abort.aborted) {
            resolve('promote');
