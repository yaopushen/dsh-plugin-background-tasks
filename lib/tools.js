...
export function registerBackgroundTools(ctx, config, seam) {
    ...
    const disposer = ctx.tools.register(defineTool({
        name: 'run_command',
        description: 'Default runner for any shell command — builds, installs, downloads, tests, training, batch scripts. ' +
                ...
                try {
                    return jobs.start({
                        ...
                        run: () => ({
                            ...
                            readOutput: () => renderProcessRead(running.readOutput(), running.sandbox, seam.escalationModes.length > 0, !settled),
                        }),
                    });
                }
                catch (err) {
