...
import { closeSync, createWriteStream, existsSync, mkdirSync, openSync, readFileSync, readSync, rmSync, statSync, } from 'node:fs';
...
export class TaskManager {
     ...
     */
    readTaskLogs(taskId, maxLines = 100) {
        const task = this.tasks.get(taskId);
        if (!task)
            return { exists: false, logs: `Task "${taskId}" not found.` };
        if (!existsSync(task.logPath))
            return { exists: true, logs: '(No log output yet)' };
        ...
        try {
            ...
            if (lines.length <= cappedLines)
                return { exists: true, logs: content };
            return {
                exists: true,
                logs: `... [showing last ${cappedLines} of ${lines.length} lines] ...\n` +
    ...
    async runCommand(request) {
        ...
        let settledResolve;
        const settledPromise = new Promise((res) => {
            settledResolve = res;
        });
        let timer;
        ...
        const settle = (code, signal, options = {}) => {
            if (settle.done)
                return;
            settle.done = true;
            if (timer !== undefined) {
        ...
        if (outcome !== 'timeout') {
            ...
            return {
                ...
                output: syncOutput,
                logPath,
                message: taskRecord.status === 'completed'
...
function describeSettlement(options, code, signal, status) {
    if (options.viaError)
        return `(spawn/process error, status: ${status})`;
