 ...
 */
export async function installPackagedPreset(ctx, presetId = 'background-shell') {
    ...
    try {
        ...
        log.warn('failed to install agent preset "%s" to %s (%s); copy preset/background-shell/ manually', presetId, targetDir, err instanceof Error ? err.message : String(err));
        return false;
    }
}
